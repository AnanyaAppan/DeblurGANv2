...
class HumanAware(Module):   
    ...
    def forward(self,img, attention_map, downsampled_attention_map):
        ...
        stacked_fg_attention = attention_map_fg.clone().detach()
        stacked_bg_attention = attention_map_bg.clone().detach()
        print(type(stacked_bg_attention))
        for i in range(7) :
            stacked_fg_attention = torch.cat((stacked_fg_attention,stacked_fg_attention),1).clone().detach()
            stacked_bg_attention = torch.cat((stacked_bg_attention,stacked_bg_attention),1).clone().detach()
